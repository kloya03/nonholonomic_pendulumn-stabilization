...
Xe = [X;cos(X(4,:));sin(X(4,:))];
Ye = [Y;cos(Y(4,:));sin(Y(4,:))];
%% Basis (lifting) functions --------------
% cubic monomial for 5 state variable +2 trignometric has 120 basis functions
tic
vn = size(Xe,1);
syms y [vn 1]
zm = monomials(y,[0:3]);
parfor i = 1:size(X,2)
    Xlift(:,i) = double(subs(zm,y,Xe(:,i)));
    Ylift(:,i) = double(subs(zm,y,Ye(:,i)));
    clc;
end
et_data_lift = toc;
...
save('koopman_roll_CS_cossine.mat')
